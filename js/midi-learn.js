...
function buildDefaultBindings(targetsById) {
    const out = {};
...
export function createMidiLearnController(opts) {
    ...
    if (loaded.shouldSeedDefaults) {
        bindings = Object.assign(Object.assign({}, buildDefaultBindings(targetsById)), bindings);
        saveRaw(bindings);
    }
    ...
    function handleMidiMessage(ev) {
        ...
        if (st >= 0xb0 && st <= 0xbf && data.length >= 3) {
            ...
            const cc = data[1];
            const val = data[2];
            setLastIncoming === null || setLastIncoming === void 0 ? void 0 : setLastIncoming(`MIDI CC ch${ch + 1} cc${cc} val${val}`);
        ...
        if (st >= 0x90 && st <= 0x9f) {
            ...
            const note = data[1];
            const vel = data.length > 2 ? data[2] : 0;
            setLastIncoming === null || setLastIncoming === void 0 ? void 0 : setLastIncoming(`MIDI NOTE ch${ch + 1} note${note} vel${vel}`);
            if (vel > 0) {
    ...
    function handleMappedCc(channel, cc, value) {
        if (channel < 0 ||
            channel > 15 ||
            cc < 0 ||
            cc > 127 ||
        ...
        setLastIncoming === null || setLastIncoming === void 0 ? void 0 : setLastIncoming(`OSC->MIDI CC ch${channel + 1} cc${cc} val${value}`);
        const data = new Uint8Array([0xb0 | (channel & 0x0f), cc, value]);
        if (tryLearnFromMessage("cc", channel, cc))
            return;
        dispatchMappedMessage(data);
    }
    function handleMappedNote(channel, note, velocity) {
        if (channel < 0 ||
            channel > 15 ||
            note < 0 ||
            note > 127 ||
        ...
        setLastIncoming === null || setLastIncoming === void 0 ? void 0 : setLastIncoming(`OSC->MIDI NOTE ch${channel + 1} note${note} vel${velocity}`);
        if (velocity > 0) {
            if (tryLearnFromMessage("note", channel, note))
                return;
        }
