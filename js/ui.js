...
function AudioMeter(analyser) {
    ...
    const gBars = barsCanvas.getContext("2d");
    const gWave = waveCanvas.getContext("2d");
    const gPeaks = peaksCanvas.getContext("2d");
    ...
    const bg = {
        low: bandCanvases.low.getContext("2d"),
        mid: bandCanvases.mid.getContext("2d"),
        high: bandCanvases.high.getContext("2d"),
    };
    ...
    setMeterAlign();
    function meterToggle(labelText, onByDefault, onChange) {
    ...
    alignSelect.addEventListener("change", () => {
        meterAlign = alignSelect.value;
        setMeterAlign();
    });
    controls.append(meterToggle("Bars", true, (on) => {
        showBars = on;
    ...
    }), meterToggle("Grid", true, (on) => (showGrid = on)), meterToggle("Scan", true, (on) => (showScan = on)), meterToggle("Split bands", true, (on) => {
        showSplitBands = on;
    ...
    function draw() {
        ...
        function drawBackdrop(gg, ww, hh) {
...
function triggerValuesForKind(kind) {
    if (kind === "trigger") {
        return { on: 127, off: 0 };
    }
    if (kind === "bool") {
...
function buildTouchOscBlueprint(state, midiCallbacks) {
    const ns = "/mmc";
    ...
    const host = state.osc.wsHost.value.trim() && !state.osc.wsHost.value.includes("://")
    ...
    const controls = midiCallbacks.getMidiTargets().map((target) => {
        ...
        if (!binding) {
            return Object.assign(Object.assign({}, base), { mode: "unmapped", osc: null });
        }
        if (binding.kind === "cc") {
            const trig = triggerValuesForKind(target.kind);
            return Object.assign(Object.assign({}, base), { mode: target.kind === "numeric" ? "fader" : "button", osc: target.kind === "numeric"
                    ? {
                        change: {
                            address: ccAddr,
        ...
        const trig = triggerValuesForKind(target.kind);
        return Object.assign(Object.assign({}, base), { mode: target.kind === "numeric" ? "fader-note" : "pad", osc: target.kind === "numeric"
                ? {
                    change: {
                        address: noteAddr,
...
function MidiDevicesPanel(clock, midiAccess, state, midiCallbacks) {
    ...
    learnBlock.append(learnHead, learnHint, learnStatus, cancelWait, mapWrap, clearAll, exportTouchOsc);
...
function OscPanel(state) {
    ...
    body.append(en, readmeRow, oscProtoRow, plainHelp, wsHead, wsGrid, udpRef, inHead, lastIn, debugHead, debugRow, debugLog, outHead, outGrid, emitRow, emitNote, st, statusFoot, hint);
