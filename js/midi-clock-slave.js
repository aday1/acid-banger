...
export function createMidiSlaveClockApi(bpm, onStatus) {
    let handler = () => { };
    let pulseIn16th = 0;
    let globalStep = 0;
    let running = true;
        ...
        if (st === MIDI_CLOCK) {
            ...
            if (lastF8Time > 0) {
                ...
                if (dt > 0.5 && dt < 250) {
                    ...
                    if (Number.isFinite(instBpm) &&
                        instBpm > 40 &&
                        instBpm < 300) {
                        bpmEma = emaAlpha * instBpm + (1 - emaAlpha) * bpmEma;
                        const [lo, hi] = bpm.bounds;
                        const clamped = Math.max(lo, Math.min(hi, Math.round(bpmEma)));
                        bpm.value = clamped;
                    }
                }
            }
...
//# sourceMappingURL=midi-clock-slave.js.map