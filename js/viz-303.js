...
function makeKnob(x, z, color = 0x262a30) {
    ...
    const skirt = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.36, 0.16, 36), new THREE.MeshStandardMaterial({
        color: 0x111318,
        roughness: 0.72,
        metalness: 0.1,
    }));
    ...
    root.add(skirt);
    const body = new THREE.Mesh(new THREE.CylinderGeometry(0.26, 0.29, 0.2, 36), new THREE.MeshStandardMaterial({
        color,
        roughness: 0.42,
        metalness: 0.38,
    }));
    ...
    root.add(body);
    const marker = new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.035, 0.2), new THREE.MeshStandardMaterial({
        color: 0xe8ddd0,
        roughness: 0.35,
        metalness: 0.2,
    }));
    ...
    root.add(marker);
    const flash = new THREE.Mesh(new THREE.TorusGeometry(0.4, 0.035, 12, 38), new THREE.MeshStandardMaterial({
        color: 0x9c6cff,
        emissive: 0x2e0f66,
...
function panelTexture() {
    ...
    for (const [txt, x, y] of knobLabels)
        g.fillText(txt, x, y);
...
export function Acid303Visual(state, analyser) {
    ...
    actions.append(recBtn, orbitToggle, orbitSpeed, camMode, trackRecBtn, vjToggle, shaderCycleBtn, vjMotionBtn, fullscreenBtn, sizeSlider, resetCam, recStatus);
