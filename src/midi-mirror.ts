...
import type { ThreeOhMachine } from "./interface.js";

export function createAcidMidiMirror(
    getOutput: () => MIDIOutput | null,
    channels: [number, number]
) {
    const lastNote: [number, number] = [-1, -1];

    function step(notes: ThreeOhMachine[], stepIndex: number) {
        const out = getOutput();
        if (!out) return;
    ...
    function allNotesOff(notes: ThreeOhMachine[]) {
        const out = getOutput();
        if (!out) return;
        notes.forEach((_, i) => {
            const ch = (channels[i] - 1) & 0x0f;
