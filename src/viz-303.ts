...
export function Acid303Visual(state: ProgramState, analyser: AnalyserNode): HTMLElement {
    ...
    function sliderWrap(label: string, input: HTMLInputElement) {
    ...
    actions.append(
        recBtn,
        orbitToggle,
        sliderWrap("Orbit", orbitSpeed),
        camMode,
        trackRecBtn,
        sliderWrap("Fly speed", flySpeed),
        sliderWrap("Fly smooth", flySmooth),
        sliderWrap("Fly focus", flyFocus),
        sliderWrap("FX size", fxSize),
        sliderWrap("FX spread", fxSpread),
        sliderWrap("FX roam", fxRoam),
        sliderWrap("FX hit", fxHit),
        vjToggle,
        shaderCycleBtn,
        vjMotionBtn,
        trippyHwBtn,
    ...
    exportGifBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            sample.width = w;
            sample.height = h;
            const sg = sample.getContext("2d", { willReadFrequently: true }) as CanvasRenderingContext2D;
            const fps = 12;
            const frameCount = 48;
    ...
    const acidHaloMeta: Array<{ baseY: number; baseR: number; phase: number; speed: number; wobble: number }> = [];
    ...
    function frame() {
        ...
        if (cameraMode === "orbit") {
            ...
            let seg = 0;
            while (seg + 1 < flyPasses.length && flyPasses[seg + 1].t < phase) seg++;
        ...
        for (let lane = 0; lane < 4; lane++) {
            ...
            if (!muted && hit > 0) drumLanePunch[lane] = 1;
            else drumLanePunch[lane] *= 0.82;
            const lampMat = drumLaneIndicators[lane].material as THREE.MeshStandardMaterial;
            lampMat.color.setRGB(
                0.3 + (drumLaneEnergy[lane] + drumLanePunch[lane] * 0.35) * 0.7,
                0.1 + (drumLaneEnergy[lane] + drumLanePunch[lane] * 0.25) * 0.2,
                0.05
            );
            lampMat.emissive.setRGB(
                0.12 + (drumLaneEnergy[lane] + drumLanePunch[lane] * 0.4) * 0.45,
                0.04 + (drumLaneEnergy[lane] + drumLanePunch[lane] * 0.3) * 0.12,
                0.02
            );
        ...
        const drumHitPulse =
            Math.max(drumLanePunch[0], drumLanePunch[1], drumLanePunch[2], drumLanePunch[3]) * 0.85 +
            drumEnergy * 0.55;
        ...
        if (perfTick % 2 === 0) {
            ...
            for (const c of patchCables) {
                const lm = c.line.material as THREE.LineBasicMaterial;
        ...
        if (trippyHardwareEnabled) {
            if (perfTick % 2 === 0) {
                ...
                for (let i = 0; i < trippyOrbs.length; i++) {
                    ...
                    o.mesh.position.set(
                        o.base.x + Math.cos(wobble * 0.8) * o.amp * acidFxSpread + Math.sin(wobble * 0.17 + i) * roam,
                        o.base.y + Math.sin(wobble) * o.amp * (1.2 + acidFxHit * 0.6),
                        o.base.z + Math.sin(wobble * 0.6) * o.amp * acidFxSpread + Math.cos(wobble * 0.19 + i) * roam
                    );
