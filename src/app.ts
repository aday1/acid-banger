...
import {
    createTransportForMode,
    type ActiveTransport,
    type TransportBundle,
    type MidiSlaveClockApi,
} from "./transport.js";
import type { TransportStepHandler } from "./interface.js";
import {
    buildMidiTargetRegistry,
...
function buildClock(
    ...
    clock: ClockUnit;
    transportRef: { current: ActiveTransport | null };
    midiSlaveClockRef: { current: MidiSlaveClockApi | null };
} {
    const bpm = parameter("BPM", [70, 200], 142);
    const currentStep = parameter("Current Step", [0, 15], 0);
    const syncMode = genericParameter<SyncMode>("sync mode", "internal");
    const midiStatusText = genericParameter<string>(
        "midi status",
        midiAccess ? "MIDI ready" : "MIDI API unavailable"
    );
    const midiDeviceEpoch = genericParameter<number>("midi epoch", 0);

    const transportRef: { current: ActiveTransport | null } = { current: null };
    const midiSlaveClockRef: { current: MidiSlaveClockApi | null } = {
        current: null,
    };

    const stepHandler: TransportStepHandler = (_t, n) => {
    ...
    function getLinkWsUrl(): string | null {
    ...
    function rebuildTransport() {
        ...
        const bundle: TransportBundle = createTransportForMode(
            syncMode.value,
            bpm,
            getMidiOutput,
            getMidiInput,
    ...
    bpm.subscribe((b) => transportRef.current?.setBpm(b));
...
async function start() {
    ...
    const midiInputId = genericParameter<string>("midi in id", "");
    const midiOutputId = genericParameter<string>("midi out id", "");
    const linkWsHost = genericParameter<string>("link ws host", "127.0.0.1");
    const linkWsPort = genericParameter<string>("link ws port", "9999");

    function getMidiOutput(): MIDIOutput | null {
    ...
    function getMidiInput(): MIDIInput | null {
        if (!midiAccess) return null;
    ...
    const { clock, midiSlaveClockRef } = buildClock(
        midiAccess,
        midiInputId,
        midiOutputId,
        linkWsHost,
    ...
    function wireMidiInput() {
        ...
        if (inp === midiInputAttached) return;
        ...
        inp.onmidimessage = (ev) => {
            ...
            if (programState.clock.syncMode.value === "midi-slave") {
                midiSlaveClockRef.current?.handleMessage(ev);
            }
        };
    }
